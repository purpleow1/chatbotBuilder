...
const SIGNUP_HREF = "/signup?next=%2Fapp%2Fbots%2Fnew";
const LOGIN_HREF = "/login?next=%2Fapp";
...
const PLAN_CONFIGS = {
  ...
  pro: {
    name: "Pro",
    price: "$29",
    description: "Launch polished bots for customers, teams, or partners.",
    features: ["10 bots", "100 documents", "2,000 monthly messages", "Remove widget branding", "Custom widget theme"]
  },
  business: {
    name: "Business",
    price: "$99",
    description: "Scale knowledge bots across departments and websites.",
    features: [
      "30 bots",
      "500 documents",
...
function MainLandingHeader({ active, className }: MainLandingHeaderProps) {
  const links = [
...
function MainDashboardScene({ className }: { className?: string }) {
...
function MainWorkflowBand({ title = "From docs to live widget in four steps" }: { title?: string }) {
  const steps = [
...
function MainFeatureGrid({ variant = "default" }: { variant?: "default" | "compact" }) {
  const features = [
...
function MainPricingSection({ headline = "Pricing that fits a real launch path" }: { headline?: string }) {
  return (
...
function MainFinalCta({ copy = "Upload five docs, test one bot, and see the widget before you think about a paid plan." }) {
  return (
    <section className="bg-slate-950 text-white">
